...
class NtkRequestIdentifierManager {
    ...
    /// 单例实例
    /// 确保全局唯一的标识符管理器
    static let shared = NtkRequestIdentifierManager()
    ...
    /// - Returns: 缓存键字符串
    func getCacheKey(request: any iNtkRequest, cacheConfig: (any iNtkRequestConfiguration)?) -> String {
...
extension NtkRequestIdentifierManager {
    ...
    ///   - cacheConfig: 缓存配置，用于参数过滤
    /// - Returns: 哈希值
    private func generateHashForCache(request: any iNtkRequest, cacheConfig: (any iNtkRequestConfiguration)?) -> Int {
        var hasher = Hasher()
