...
extension NtkNetwork {
    ...
    /// - Returns: 返回自身以支持链式调用
    @discardableResult
    func retry(_ retryInterceptor: NtkRetryInterceptor) -> Self {
    ...
    /// - Returns: 返回自身以支持链式调用
    @discardableResult
    func retry(_ retryPolicy: iNtkRetryPolicy) -> Self {
