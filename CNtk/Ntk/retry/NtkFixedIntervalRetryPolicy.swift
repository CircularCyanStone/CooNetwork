...
/// 固定间隔重试策略
/// 每次重试使用相同的延迟时间
struct NtkFixedIntervalRetryPolicy: iNtkRetryPolicy {
    /// 最大重试次数
...
extension NtkFixedIntervalRetryPolicy {
    /// 快速重试策略：最多3次，间隔1秒
    static var fast: NtkFixedIntervalRetryPolicy {
    ...
    /// 标准重试策略：最多5次，间隔2秒
    static var standard: NtkFixedIntervalRetryPolicy {
        return NtkFixedIntervalRetryPolicy(
            maxRetryCount: 5,
            interval: 2.0,
            jitterFactor: 0.2
    ...
    /// 慢速重试策略：最多7次，间隔5秒
    static var slow: NtkFixedIntervalRetryPolicy {
        return NtkFixedIntervalRetryPolicy(
            maxRetryCount: 7,
            interval: 5.0,
            jitterFactor: 0.3
    ...
    /// 无抖动重试策略：精确的固定间隔
    static func precise(maxRetryCount: Int = 3, interval: TimeInterval = 2.0) -> NtkFixedIntervalRetryPolicy {
        return NtkFixedIntervalRetryPolicy(
