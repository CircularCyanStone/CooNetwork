...
3. **⚠️ 在async函数中使用阻塞操作（重要）**
```swift
// ❌ 严重错误：阻塞线程的同步操作
func blockingMethod() async -> Bool {
    Thread.sleep(forTimeInterval: 1.0)  // 阻塞当前线程
    return true
}

// ❌ 其他阻塞操作示例
func badExamples() async {
    // 同步文件I/O
    let data = FileManager.default.contents(atPath: "file.txt")
...
func cooperativeMethod() async -> Bool {
    try? await Task.sleep(nanoseconds: 1_000_000_000)  // 挂起任务，释放线程
    return true
}

func goodExamples() async {
    // 异步文件I/O
...
1. **线程池资源有限**
   - Swift全局并发池通常只有CPU核心数个线程
   - 阻塞操作占用线程但不释放，导致可用线程急剧减少
...
2. **并发性能退化**
   ...
   for _ in 0 ..< 10000 {
       Task {
           _ = await blockingMethod()  // 每个调用阻塞线程1秒
           print("Done")
       }
   }
   // 结果：10000个任务可能需要几千秒才能完成！
   ```

3. **调度机制失效**
   - Swift并发依赖协作式调度
   - 阻塞操作破坏了这种协作，导致调度器无法有效管理任务

#### 🔍 Thread.sleep vs Task.sleep 对比

| 操作 | 线程状态 | 其他任务 | 性能影响 |
|------|----------|----------|----------|
| `Thread.sleep(1.0)` | 阻塞，不可用 | 必须等待其他线程 | 严重性能退化 |
| `Task.sleep(nanoseconds: 1_000_000_000)` | 释放，可复用 | 可以立即执行 | 最优性能 |

#### 📊 性能测试数据

基于实际测试（参见 `阻塞vs协作式挂起演示.swift`）：

```
5个并发任务，每个等待1秒：
- 使用Thread.sleep: 总耗时 ≈ 5秒（串行执行）
- 使用Task.sleep: 总耗时 ≈ 1秒（并行执行）
