...
typedef NS_ENUM(NSInteger, mPaaSBizType)
{
    ...
    mPaaSAPMOffinePKGiOS,   // APM离线包
    mPaaS_GMSSL_iOS,         // GMSSL
    mPaaS_ZOLOZ_iOS,         // 人脸
    mPaaS_AOP_iOS,           // 隐私切点
    mPaaS_MTR_iOS           // 网络探测MTR
};
...
/**
 APM网络RPC日志上报
 */
+ (void)mPaaSAPMRPCNetLog:(mPaaSBizType)bizType extDict:(NSDictionary*)extDict identifier:(NSString *)identifier;
...
/**
  自动化埋点设置自定义的扩展参数
 */
+ (void)setMPaaSAutoExtendParam:(NSDictionary *)extParam;

/**
  获取自动化埋点自定义的扩展参数
 */
+ (NSDictionary *)mPaaSAutoExtendParam;
