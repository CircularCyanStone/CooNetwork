...
# define SSL_KEYLESS_TYPE_RSA_NO_PSS_FLAG              0x00010000
# define SSL_KEYLESS_TYPE_RSA_PSS_DEFAULT_FLAG         0x00020000
# define SSL_KEYLESS_TYPE_RSA_PSS_FLAG                 0x00040000
# define SSL_KEYLESS_TYPE_RSA_NO_PADDING_FLAG          0x00080000
#endif
...
#define SSL_set_keyless_rsa_flag BABASSL_set_keyless_rsa_flag

void BABASSL_set_keyless_rsa_flag(SSL *s, int flag);
...
#define SSL_set_lurk_rsa_flag BABASSL_set_lurk_rsa_flag

void BABASSL_set_lurk_rsa_flag(SSL *s, int flag);
...
int SSL_get_handshake_hash(SSL *s, unsigned char *out, size_t outlen,
                           size_t *hashlen);
...
/*
 * Just define the macro.
 */
# if !(defined(OPENSSL_NO_KEYLESS) && defined(OPENSSL_NO_LURK))
#  define SSL_OP_NO_RSA_PSS                              0
# endif
...
# ifndef OPENSSL_NO_OPTIONS2
#  ifndef OPENSSL_NO_KEYLESS
#   define SSL_OP2_KEYLESS                               0x00000001
#   define SSL_OP2_KEYLESS_NTLS                          0x00000002
#  endif

#  ifndef OPENSSL_NO_LURK
#   define SSL_OP2_LURK                                  0x00000004
#   define SSL_OP2_LURK_NTLS                             0x00000008
#   define SSL_OP2_DISABLE_EXTENDED_MASTER_SECRET        0x00000010
#  endif
# endif
...
# ifndef OPENSSL_NO_OPTIONS2
uint64_t SSL_CTX_get_options2(const SSL_CTX *ctx);
uint64_t SSL_get_options2(const SSL *s);
uint64_t SSL_CTX_clear_options2(SSL_CTX *ctx, uint64_t op);
uint64_t SSL_clear_options2(SSL *s, uint64_t op);
uint64_t SSL_CTX_set_options2(SSL_CTX *ctx, uint64_t op);
uint64_t SSL_set_options2(SSL *s, uint64_t op);
# endif
...
#ifndef OPENSSL_NO_EARLYDATA_REJECT_TOLERANCE
void SSL_CTX_set_earlydata_tolerance_time(SSL_CTX *ctx, int time);
void SSL_set_earlydata_tolerance_time(SSL *s, int time);
#endif
...
void SSL_set_quic_early_data_enabled2(SSL *ssl, int enabled);
__owur int SSL_set_quic_early_data_context(SSL *ssl,
                                           const uint8_t *context,
                                           size_t context_len);

/* here we compatiable with boringssl */
void SSL_set_quic_use_legacy_codepoint(SSL *ssl, int use_legacy);
# endif

# ifndef OPENSSL_NO_COMPATIBLE_BORINGSSL
typedef enum {
  ssl_verify_ok,
  ssl_verify_invalid,
  ssl_verify_retry
...
void SSL_CTX_set_custom_verify(SSL_CTX *ctx, int mode,
         ssl_verify_result_t (*callback)(SSL *ssl, uint8_t *out_alert));
