...
@TFNActor
protocol iTFNCacheStorage: Sendable {
    
    /// 添加请求到缓存管理
    /// 用于注册需要缓存的请求，可用于预处理或统计
    /// - Parameter request: 网络请求对象
    func addRequest(_ request: TFNMutableRequest)
    ...
    /// - Returns: 存储操作是否成功
    func setData(metaData: TFNCacheMeta, key: String) async -> Bool
    
    /// 从缓存获取数据
    /// 根据缓存键检索对应的缓存数据和元信息
    /// - Parameter key: 缓存键
    /// - Returns: 缓存的元数据对象，如果缓存不存在则返回nil
    func getData(key: String) async -> TFNCacheMeta?
