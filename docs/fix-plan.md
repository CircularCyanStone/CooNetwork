...
#### 3.1 优化 NtkNetwork Actor 隔离 ⚠️ 跳过

**问题描述：** 使用 `@unchecked Sendable` 和内部锁，违反 Swift 6 Actor 隔离原则。

**跳过原因：**
1. 当前设计是 Configurator + Executor 分离模式，执行层已经是 Actor
2. 每次执行通过快照机制读取配置，实际安全
3. 改为 Actor 方案属于破坏性变更，需要修改所有用户代码
4. 编译无警告，测试通过
5. 理论"最佳实践" < 实际破坏性影响
