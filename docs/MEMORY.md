...
- **问题**: 排序逻辑分散在 5 个地方（NtkNetwork 中 2 个未使用的计算属性，NtkNetworkExecutor 中 3 处）
- **改进**:
  - 删除 NtkNetwork 中未使用的 `interceptors` 和 `coreInterceptors` 排序计算属性
  - 在 NtkNetworkExecutor 中提取 `sortInterceptors(_:)` 私有方法统一排序逻辑
  - 拦截器执行顺序保持不变：用户拦截器（原始顺序）→ core 拦截器（按优先级排序）
- **状态**: ✅ 已完成 (2026-03-17)

### 8. 重试逻辑冗余检查
- **文件**: `NtkRetryInterceptor.swift`
- **完整评估**:
  - **问题 1（严重程度：高）**: 检查 2（59-62 行）是死代码
    - 原因：while 条件 `attemptCount < maxRetryCount` 已经在检查，进入 catch 时 `attemptCount` 最大值是 `maxRetryCount - 1`
    - 修复：删除冗余检查（59-62 行）
    - 状态：✅ 已修复
  - **问题 2（严重程度：低）**: 错误处理代码重复 2 处（53-56 行、65-68 行）
    - 评估：重复代码仅 2 处，每处 2 行，修复后可读性反而下降（需跳转到方法定义）
    - 决定：❌ 不修复
  - **问题 3（严重程度：无）**: 循环结束后的处理（81-84 行）
    - 评估：这是独立的处理路径，不是重复代码
    - 决定：❌ 不是问题
- **验证**: 运行所有测试通过（20 个测试），包括边界情况 `maxRetryCount = 0`
- **状态**: ✅ 已完成 (2026-03-18)
